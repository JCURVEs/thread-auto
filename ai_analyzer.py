...
- **Single (단일)**:
    - **기업 제휴/협력/계약 뉴스 (예: Microsoft-Varaha 계약)** -> **반드시 Single로 작성**.
    - 단순 기능 업데이트, 숫자/실적 발표.
    - **핵심 내용 전달에 집중**. 불필요하게 늘리지 말 것.
- **Multi (스레드)**:
    - **거대 플랫폼/모델 런칭** (GPT-5급). **기술적 분석이 매우 깊게 필요한 경우**에만 제한적으로 사용.
    - 일반적인 뉴스는 Single이 원칙. (Multi 남발 금지)
...
2. **메인 포스트 (Main Post)**:
    - **제목**: **[핵심을 찌르는 소제목]** 형태로 작성. (Single, Multi 모두 필수). 
    - **형식**: `main_post`의 **맨 앞**에 반드시 `**[제목]**`을 적고 시작할 것.
