...
## [arXiv NLP] LLM의 외부 증거 통합 메커니즘에 관한 분포 이론 및 실험적 검증
...
**요약:**  
외부 도구나 검색 증강 생성(RAG)을 통해 공급된 증거가 LLM의 기존 판단에 어떻게 통합되는지를 설명하는 분포 이론을 제시합니다. 4개 계열의 12개 LLM과 8개 도메인에서 1,000만 회 이상의 실험을 수행하여, 수신자(모델)의 선험적 확률과 증거의 기울기가 답변 분포를 이동시킨다는 세 가지 예측을 확인했습니다. 특히 모델이 내부적으로 증거의 무효를 검증했음에도 불구하고 93~100%의 확률로 해당 증거를 답변에 통합하는 현상을 발견했습니다. 인과적 개입 분석을 통해 증거 통합이 네트워크 후반부에서 구조화된 단계로 구현되며, 검증 표현과 후보 통합 상태가 완전히 분리되어 있음을 밝혔습니다.

**쉬운설명:**  
즉, AI가 이미 내린 결론이 있어도 외부에서 들어온 정보가 그 결론에 맞춰 변형되어 최종 답변에 반영되는 과정이, 단순히 정보를 믿는 것이 아니라 모델 내부의 기존 확률 분포를 이동시키는 기계적 과정임을 보여준 연구입니다.
