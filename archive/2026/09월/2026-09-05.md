...
## [arXiv AI] 멀티스텝 선행 예측을 통한 도구 사용 에이전트 지연 시간 단축 기법 SMC 공개

**분야:** 에이전트/자동화 | **중요도:** 7점

**요약:**  
도구 사용 LLM 에이전트의 병목인 직렬적 행동-관찰 순환을 해결하기 위해 '선행 예측 매크로 커밋(SMC)' 런타임 메커니즘을 제안했습니다. 이 기법은 대형 액터 모델이 공식 궤적을 생성하는 동안, 빠른 드래프터 모델이 격리된 환경에서 미래 행동 체인을 미리 예측하고 실행합니다. 학습 트레이스에서 추출한 반복적인 다중 행동 골격을 매크로 라이브러리에 저장하여 런타임 매칭에 활용하며, 액터의 다음 도구 호출이 일치하면 미리 실행된 단계를 공식 궤적으로 커밋합니다. Qwen3.5-27B INT4와 Qwen3.5-4B 모델을 사용해 기존 선행 예측 기법 대비 10.23%, 순차 실행 대비 18.59%의 지연 시간을 줄이는 성능을 입증했습니다.

**쉬운설명:**  
즉, 요리사가 메인 요리를 만드는 동안 보조 요리사가 미리 재료를 손질해 두어 전체 식사 준비 시간을 단축하는 것과 같은 원리입니다.
