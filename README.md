...
원문 발행일을 기준으로 최근 48시간의 기사만 수집합니다. HTML 목록의 수집 시각이나
수정일을 발행일로 사용하지 않으며, RSS와 원문 발행일이 다르면 더 오래된 날짜를 적용합니다.
발행일을 확인할 수 없거나 미래 날짜인 항목은 제외합니다. 아카이브에는 원문발행일과 수집일을 구분해 표시합니다.

전체 아카이브의 URL을 비교하고 추적 매개변수와 URL fragment를 제거해 중복 분석을 줄입니다.
날짜 확인과 본문 추출은 한 번 받은 페이지를 재사용합니다. AI 입력에서는 HTML, 반복 문단,
본문과 겹치는 RSS 요약을 제거합니다. 고정 프롬프트는 2026-09-09 수정 기준 문자 수가 23.6% 감소했으며,
실제 토큰 절감률과 서버 캐시 적중률은 모델 및 입력 내용에 따라 달라집니다.

기존 날짜 오류 점검은 `python scripts/audit_archive_dates.py --dates 2026-09-09`로 실행합니다.
`--apply`를 지정하면 확실히 오래된 항목을 일일 Markdown에서 제외하고 원본을
`archive/excluded/`의 JSONL에 보존합니다. 날짜 확인이 불가능한 기존 항목은 자동 삭제하지 않습니다.
